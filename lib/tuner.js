    ...
    files.forEach(
        function(f) {            
            var fileWorkflow = function(done) {
                flower.series(
                    [self.prepareFile(f), self.convertFile(f), self.embedArtworkToFile(artwork, f)], 
                    function(err, result) {
                        if (err) {
                            done(err);
                        } else {
...
Tuner.prototype.prepareFile = function(file) {
    var self = this;
    return function(done) {
        self.emit('preparingFile', file);
...
Tuner.prototype.convertFile = function(file) {
    ...
    return function(done) {
        ...
        xld.on('error', done);
        xld.convert(file, {format: Xld.formats.alac, output: path.dirname(file)});
    }
};
...
Tuner.prototype.embedArtworkToFile = function(artwork, file) {
    var self = this;
    return function(done) {
        var filename = file.replace('.flac', '.m4a');
