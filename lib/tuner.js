...
Tuner.prototype.processDirectory = function(dir) {
    ...
    files.forEach(
        function(f) {
            var prepareFile = function(done) {
                self.emit('preparingFile', f);
                done(null, 'filePrepared');
            };

            var convertFile = function(done) {
                var xld = new Xld({debug: self.debug});
                xld.on('trackConverted', function(result) {
                    self.emit('trackConverted', f);
                    done(null, result);
                });
                xld.on('error', done);
                xld.convert(f, {format: Xld.formats.alac, output: path.dirname(f)});
            };

            var embedArtworkToFile = function(done) {
                var filename = f.replace('.flac', '.m4a');
            ...
            var fileWorkflow = function(done) {
                flower.series([prepareFile, convertFile, embedArtworkToFile], function(err, result) {
                    if (err) {
                        done(err);
                    } else {
