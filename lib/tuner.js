...
Tuner.prototype.processDirectory = function(dir) {
    ...
    async.waterfall([
        ...
        function cleanUp(done) {
            ...
            fs.writeFileSync(path.resolve(dir, path.separator, '.processed'), '', 'utf-8');
...
Tuner.prototype.cleanDirectory = function(dir, done) {
    debug("processing the directory '%s'", dir)
    ...
    files.forEach(
        function(f) {
            if (self.dryRun) {
                ...
                itunes.add(path.resolve(dir, path.separator, f));
            }
        }
    );
};
...
Tuner.prototype.embedArtworkToFile = function(artwork, file) {
    ...
    return function(file, done) {
        var filename = path.resolve(file.directory, path.separator, file.filename.replace(FILE_EXTENSION_REGEXP, '.m4a'));
    ...
    files.forEach(function(f) {
        if (filter.test(f)) {
            ...
            if (stats.isDirectory()) {
                result = result.concat(getFilesToBeProcessed(path.resolve(dir, path.separator, f), regexp));
            }
        }
    });
